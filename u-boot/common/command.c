	...
	for (i = 1; i < argc; ++i) {
		if ((cmdtp = find_cmd(argv[i])) != NULL) {
			...
			if (cmdtp->help) {
				...
				puts ("- there is no help available for this command\n");
				rcode = 1;
			}
			putc ('\n');
			...
			printf("Unknown command '%s' - try 'help' without arguments for list of available commands\n\n", argv[i]);
			rcode = 1;
		}
	}
	return rcode;
...
U_BOOT_CMD(
			...
			"\t'help' prints help for the monitor commands.\n"
