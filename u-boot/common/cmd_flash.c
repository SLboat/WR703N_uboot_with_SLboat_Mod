...
static int abbrev_spec(char *str, flash_info_t ** pinfo, int *psf, int *psl) {
	...
	if ((p = strchr(str, ':')) == NULL) return 0;
	*p++ = '\0';

	bank = simple_strtoul(str, &ep, 10);
	if (ep == str || *ep != '\0' || bank < 1 || bank > CFG_MAX_FLASH_BANKS || (fp = &flash_info[bank - 1])->flash_id == FLASH_UNKNOWN) return -1;

	str = p;
	if ((p = strchr(str, '-')) != NULL) *p++ = '\0';

	first = simple_strtoul(str, &ep, 10);
	if (ep == str || *ep != '\0' || first >= fp->sector_count) return -1;

	if (p != NULL) {
		last = simple_strtoul(p, &ep, 10);
		if (ep == p || *ep != '\0' || last < first || last >= fp->sector_count) return -1;
	} else {
		last = first;
	}
	...
	*s_count = 0;
	...
	for (bank = 0, info = &flash_info[0]; (bank < CFG_MAX_FLASH_BANKS) && (addr_first <= addr_last); ++bank, ++info) {
		...
		for (sect = 0; sect < info->sector_count; ++sect) {
			...
			if (addr_first > end) continue;
			if (addr_last < info->start[sect]) continue;
	...
	if (planned && (rcode == 0)) {
		for (bank = 0, info = &flash_info[0]; (bank < CFG_MAX_FLASH_BANKS) && (rcode == 0); ++bank, ++info) {
			if (s_first[bank] >= 0) {
				...
				/*
				printf("s_last[bank]: %ld\n", s_last[bank]);
				printf("info->sector_count: %ld\n", info->sector_count);
				printf("info->size: %ld\n", info->size);
				printf("info->start[0]: %ld\n", info->start[0]);
				printf("info->start[s_last[bank] + 1]: %ld\n", info->start[s_last[bank] + 1]);
				*/
