...
int _do_setenv(int flag, int argc, char *argv[]) {
	...
	for (env = env_data; *env; env = nxt + 1) {
		for (nxt = env; *nxt; ++nxt)
			;
		if ((oldval = envmatch((uchar *) name, env - env_data)) >= 0) break;
	}
	...
	if (oldval >= 0) {
		...
		if (console != -1) {
			...
			if (console_assign(console, argv[2]) < 0) return 1;
		...
		if (strcmp(argv[1], "baudrate") == 0) {
			...
			for (i = 0; i < N_BAUDRATES; ++i) {
				if (baudrate == baudrate_table[i]) break;
			}
			if (i == N_BAUDRATES) {
				printf("## Error: baudrate %d bps not supported\n", baudrate);
				return 1;
			}
			printf("## Error: switch baudrate to %d bps and press ENTER...\n", baudrate);
			udelay(50000);
...
char *getenv(char *name) {
	...
	for (i = 0; env_get_char(i) != '\0'; i = nxt + 1) {
		...
		if ((val = envmatch((uchar *) name, i)) < 0) continue;
...
static int envmatch(uchar *s1, int i2) {
	...
	while (*s1 == env_get_char(i2++))
		if (*s1++ == '=') return (i2);
	if (*s1 == '\0' && env_get_char(i2 - 1) == '=') return (i2);
	return (-1);
}
...
"\t- delete environment variable 'name'\n");
    ...
    ((CONFIG_COMMANDS & (CFG_CMD_ENV|CFG_CMD_NAND)) == \
      (CFG_CMD_ENV|CFG_CMD_NAND))
U_BOOT_CMD( saveenv, 1, 0, do_saveenv, "save environment variables to persistent storage\n", NULL);
