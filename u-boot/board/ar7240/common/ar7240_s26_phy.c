...
typedef enum {
	PHY_SRCPORT_INFO, PHY_PORTINFO_SIZE,
} PHY_CAP_TYPE;

typedef enum {
	PHY_SRCPORT_NONE, PHY_SRCPORT_VLANTAG, PHY_SRCPORT_TRAILER,
} PHY_SRCPORT_TYPE;
...
{ TRUE, /* port 1 -- LAN port 1 */
	FALSE, ENET_UNIT_LAN, 0, ATHR_PHY0_ADDR, ATHR_LAN_PORT_VLAN },

										{ TRUE, /* port 2 -- LAN port 2 */
											FALSE, ENET_UNIT_LAN, 0, ATHR_PHY1_ADDR, ATHR_LAN_PORT_VLAN },

										{ TRUE, /* port 3 -- LAN port 3 */
											FALSE, ENET_UNIT_LAN, 0, ATHR_PHY2_ADDR, ATHR_LAN_PORT_VLAN },

										{ TRUE, /* port 4 --  LAN port 4 */
											FALSE, ENET_UNIT_LAN, 0, ATHR_PHY3_ADDR, ATHR_LAN_PORT_VLAN /* Send to all ports */
										},

										{ TRUE, /* port 5 -- WAN Port 5 */
											FALSE, ENET_UNIT_WAN, 0, ATHR_PHY4_ADDR, ATHR_LAN_PORT_VLAN /* Send to all ports */
										},

										{ FALSE, /* port 0 -- cpu port 0 */
											TRUE, ENET_UNIT_LAN, 0, 0x00, ATHR_LAN_PORT_VLAN },
...
/* Forward references */BOOL athrs26_phy_is_link_alive(int phyUnit);
...
void athrs26_reg_init_lan(void) {
	...
	while (i--) {
		if (!is_ar933x())
		sysMsDelay(100);
		if (!(athrs26_reg_read(0x0) & 0x80000000)) break;
	}
 ...
 */BOOL athrs26_phy_is_link_alive(int phyUnit) {
	...
	if (phyHwStatus & ATHR_STATUS_LINK_PASS) return TRUE;
...
int athrs26_phy_is_fdx(int ethUnit) {
	...
	if (ethUnit == ENET_UNIT_LAN) return TRUE;
	...
	for (phyUnit = 0; phyUnit < ATHR_PHY_MAX; phyUnit++) {
		...
		if (athrs26_phy_is_link_alive(phyUnit)) {
			...
			if (phyHwStatus & ATHER_STATUS_FULL_DEPLEX) return TRUE;
		}
	}
...
int athrs26_phy_speed(int ethUnit) {
	...
	if (ethUnit == ENET_UNIT_LAN) return _1000BASET;
...
int athrs26_mdc_check(void) {
	...
	for (i = 0; i < 4000; i++) {
		if (athrs26_reg_read(0x10c) != 0x18007fff) return -1;
	}
	return 0;
}
