...
static void nc_handler(uchar * pkt, unsigned dest, unsigned src, unsigned len) {
	if (input_size) NetState = NETLOOP_SUCCESS; /* got input - quit net loop */
}
...
int nc_input_packet(uchar * pkt, unsigned dest, unsigned src, unsigned len) {
	...
	if (dest != nc_port || !len) return 0; /* not for us */

	if (input_size == sizeof input_buffer) return 1; /* no space */
	if (len > sizeof input_buffer - input_size) len = sizeof input_buffer - input_size;

	end = input_offset + input_size;
	if (end > sizeof input_buffer) end -= sizeof input_buffer;
...
static void nc_send_packet(const char *buf, int len) {
	...
	if (!memcmp(nc_ether, NetEtherNullAddr, 6)) {
		if (eth->state == ETH_STATE_ACTIVE) return; /* inside net loop */
		output_packet = buf;
...
int nc_start(void) {
	...
	if (getenv("serverip")) {
		...
		if (!nc_ip) return -1; /* ncip is 0.0.0.0 */
		//if ((p = strchr(getenv("ncip"), ':')) != NULL) nc_port = simple_strtoul(p + 1, NULL, 10);
		if ((p = strchr(getenv("serverip"), ':')) != NULL) nc_port = simple_strtoul(p + 1, NULL, 10);
	} else {
...
int nc_tstc(void) {
	...
	eth = eth_get_dev();
	if (eth && eth->state == ETH_STATE_ACTIVE) return 0; /* inside net loop */
